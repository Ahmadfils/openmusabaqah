...
import { Trash2, Edit3, Plus, Save, X, Loader2 } from 'lucide-react';
...
export default function AdminPage() {
    ...
    const [qMaxAyah, setQMaxAyah] = useState(7); // Default Fatiha
    ...
    const fetchParticipants = async (groupId?: string) => {
        const url = groupId ? `/api/admin/participants?groupId=${groupId}` : '/api/admin/participants';
    ...
    const deleteGroup = async (id: string) => {
        if (!confirm('Are you sure you want to delete this group? (It must be empty)')) return;
    ...
    const createQuestion = async () => {
        if (!qSurahEn || !qText || !selectedGroupId) {
            alert('Please fill in all required fields (Surah, Question Text, and Group)');
            return;
        }
        ...
        if (res.ok) {
            ...
            const err = await res.json();
            alert(`Failed to create question: ${err.error || 'Unknown error'}`);
        }
    };
    ...
    const updateQuestion = async () => {
        ...
        if (res.ok) {
            ...
            const err = await res.json();
            alert(`Failed to update question: ${err.error || 'Unknown error'}`);
        }
    };
    ...
    const createParticipant = async () => {
        ...
        if (res.ok) {
            ...
            const err = await res.json();
            alert(`Failed to add participant: ${err.error || 'Unknown error'}`);
        }
    };
    ...
    const updateParticipant = async () => {
        ...
        if (res.ok) {
            ...
            const err = await res.json();
            alert(`Failed to update participant: ${err.error || 'Unknown error'}`);
        }
    };
        ...
        <main className="min-h-screen p-6 md:p-10">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="flex gap-4 mb-8 overflow-x-auto pb-2">
                    {['groups', 'participants', 'questions'].map((tab) => (
                        <button
                            key={tab}
                    ...
                    {activeTab === 'groups' && (
                        <div className="animate-slide-up">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {groups.map((group) => (
                                    <div key={group.id} className="glass-card p-6 border-turquoise-500/10 hover:border-turquoise-500/30 transition-all flex flex-col h-full">
                            ...
                            <div className="glass-card p-8 mb-8">
                                <div className="flex justify-between items-center mb-6">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <label className="text-xs text-turquoise-400 font-bold uppercase tracking-wider">Show Group:</label>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div className="space-y-4">
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                <label className="admin-label">Question Number</label>
                                                <input type="number" value={qNumber} onChange={(e) => setQNumber(Number(e.target.value))} className="admin-input w-full" />
                                            </div>
                                            <div>
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <select
                                                    value={qStartAyah}
                                                    onChange={(e) => setQStartAyah(Number(e.target.value))}
                                                    className="admin-input w-full"
                                                >
                                            ...
                                            <div>
                                                ...
                                                <select
                                                    value={qEndAyah}
                                                    onChange={(e) => setQEndAyah(Number(e.target.value))}
                                                    className="admin-input w-full"
                                                >
                                        ...
                                        <div>
                                            <label className="admin-label">Topic / Context</label>
                                            <input type="text" placeholder="e.g. Rules of Fasting" value={qTopic} onChange={(e) => setQTopic(e.target.value)} className="admin-input w-full" />
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            ...
                                            <textarea value={qText} onChange={(e) => setQText(e.target.value)} rows={3} className="admin-input w-full resize-none" placeholder="Recite from verse X to Y..." />
                                        </div>
                                        <div>
                                            <label className="admin-label">Expected Answer / Notes</label>
                                            <textarea value={qAnswer} onChange={(e) => setQAnswer(e.target.value)} rows={3} className="admin-input w-full resize-none" placeholder="Key segments or rulings..." />
                                        </div>
                                        <div className="flex items-end gap-4">
                                            <div className="flex-1">
                                                <label className="admin-label">Max Points</label>
                                                <input type="number" value={qPoints} onChange={(e) => setQPoints(Number(e.target.value))} className="admin-input w-full" />
                                            </div>
                                            <button onClick={createQuestion} className="btn-gold flex-1 py-2.5 flex items-center justify-center gap-2"><Plus size={18} /> Add Question</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div key={q.id} className="glass-card p-6 border-white/5 hover:border-white/10 transition-all">
                                        {editingQuestion?.id === q.id ? (
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <div className="space-y-3">
                                                    <div className="grid grid-cols-3 gap-2">
                                                        <input type="number" value={editingQuestion.number} onChange={(e) => setEditingQuestion({ ...editingQuestion, number: Number(e.target.value) })} className="admin-input py-1" placeholder="#" />
                                                        <select
                                                            value={editingQuestion.surahNumber - 1}
                                                            onChange={(e) => onEditingSurahChange(Number(e.target.value))}
                                                            className="admin-input py-1 col-span-2"
                                                        >
                                                            {surahList.map((s, idx) => (
                                                    ...
                                                    <div className="grid grid-cols-2 gap-2">
                                                        ...
                                                        >
                                                            {Array.from({ length: editingSurahMaxAyah || q.surahDetails?.totalAyah || 286 }, (_, i) => (
                                                        ...
                                                        <select
                                                            ...
                                                            className="admin-input py-1"
                                                        >
                                                            {Array.from({ length: (editingSurahMaxAyah || q.surahDetails?.totalAyah || 286) - editingQuestion.startAyah + 1 }, (_, i) => (
                                                                <option key={i + editingQuestion.startAyah} value={i + editingQuestion.startAyah}>{i + editingQuestion.startAyah}</option>
                                                            ))}
                                                        </select>
                                                    </div>
                                                </div>
                                                <div className="space-y-3">
                                                    <textarea value={editingQuestion.questionText} onChange={(e) => setEditingQuestion({ ...editingQuestion, questionText: e.target.value })} className="admin-input py-1 w-full text-xs" rows={2} />
                                                    <div className="flex gap-2">
                                            ...
                                            <div className="flex flex-col md:flex-row gap-6">
                                                <div className="flex-none flex items-center justify-center bg-gold-500/10 rounded-2xl w-16 h-16 border border-gold-500/20">
                                                    <span className="text-2xl font-bold text-gold-400">#{q.number}</span>
                                                </div>
                                                <div className="flex-1">
                                                    <div className="flex justify-between items-start">
                                                        <div>
                                                            <div className="flex items-center gap-3">
                                                                <h3 className="text-lg font-bold text-white">{q.surah}</h3>
                                                                <span className="font-arabic text-turquoise-400 text-lg">{q.surahArabic}</span>
                                                            </div>
                                                            <p className="text-xs text-turquoise-500 uppercase tracking-widest font-bold">
                                                                Verses: {q.startAyah || '?'}-{q.endAyah || '?'} • {q.difficulty} • {q.points} pts
                                                            </p>
                                                        </div>
                                                        <div className="flex gap-2">
                                                            <button
                                                                ...
                                                                className="text-turquoise-500 hover:text-white p-2"
                                                            >
                                                                <Edit3 size={18} />
                                                            </button>
                                                            <button onClick={() => deleteQuestion(q.id)} className="text-red-500/70 hover:text-red-400 p-2"><Trash2 size={18} /></button>
                                                        </div>
                                                    </div>
                                                    <p className="text-turquoise-300/80 text-sm mt-3 border-l-2 border-gold-500/30 pl-4 py-1 italic">{q.questionText}</p>
                                                </div>
                                            </div>
                                        )}
                                    </div>
                                ))}
                                {questions.length === 0 && <div className="p-12 text-center text-turquoise-700 italic glass-card">No questions added yet for this group.</div>}
                            </div>
                        </div>
                    )}
                </div>
            ...
            <style jsx global>{`
                .admin-input {
                    ...
                    background-color: #064e3b; /* Deep teal/turquoise */
                }
                .admin-input:focus {
                ...
                .admin-input option {
                    @apply bg-turquoise-950 text-white;
